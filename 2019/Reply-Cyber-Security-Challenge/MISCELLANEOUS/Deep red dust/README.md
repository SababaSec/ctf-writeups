...
However, looking at the hex dump, we see the following:
...
The first four bytes are set to `RBOY`, which is a non-standard header. But the string `IHDR` suggests a PNG image.

We used `hexedit` to replace the first four bytes with `89 50 4E 47`, which are the PNG magic bytes.
...
The ZIP file detected by the `file` command is embedded in the image and can be extracted using `binwalk` with the `-e` flag. The ZIP file was password-protected, and the password was, of course, `K33pItS3cr3t`.
...
We can conclude that the macro takes input from a text box and XORs it with the encrypted flag. Since we know the flag format, we can use this knowledge to obtain the first five letters from the key.
...
The first five characters of the key are `'Oppor'`. Judging by the Mars-themed challenge, we suspected that the key might be `'Opportunity'`. This turned out to be correct:
